...
// Social provider enum for type-safe provider values
export const socialProviderEnum = ['apple', 'google'] as const;
export type SocialProvider = (typeof socialProviderEnum)[number];

// User model - Custom Auth
// IMPORTANT: Exactly one auth method must be set: passwordHash OR (socialProvider + socialId)
// This invariant must be enforced at the application layer
export const users = sqliteTable(
  'users',
  {
    ...
    socialProvider: text('social_provider', { enum: socialProviderEnum }), // 'apple', 'google', or NULL for password
  ...
  (table) => ({
    ...
    // Unique constraint on (socialProvider, socialId) prevents duplicate social auth accounts
    socialUnique: unique('users_social_unique').on(table.socialProvider, table.socialId),
...
export const sessions = sqliteTable(
  ...
  (table) => ({
    ...
    // Unique constraint on (userId, deviceId) ensures one session per device
    userDeviceUnique: unique('sessions_user_device_unique').on(table.userId, table.deviceId),
    userIdExpiresAtIdx: index('sessions_user_expires_idx').on(table.userId, table.expiresAt),
    // Index on expiresAt for efficient cleanup of expired sessions
...
export const mfaSecrets = sqliteTable(
  'mfa_secrets',
  {
    ...
    userId: text('user_id')
      ...
      .unique()
      .references(() => users.id, { onDelete: 'cascade' }),
    secret: text('secret').notNull(), // TOTP secret - MUST be encrypted at application layer before storing
