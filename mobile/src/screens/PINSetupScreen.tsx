...
export function PINSetupScreen({ onSuccess }: PINSetupScreenProps): React.JSX.Element {
  ...
  const handlePinSubmit = async () => {
    ...
    if (step === 'create') {
      ...
      // Store the PIN securely using the auth service
      setIsStoring(true);
      try {
        ...
        Alert.alert(
          'Error',
          error instanceof Error ? error.message : 'Failed to store PIN. Please try again.'
        );
        setStep('create');
        setPin('');
        setConfirmPin('');
        ...
        <View style={styles.keypad}>
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <Pressable
              ...
              onPress={() => !isStoring && handlePinChange(currentPin + num.toString())}
              disabled={isStoring}
            >
          ...
          <Pressable
            style={styles.keypadButton}
            onPress={() => !isStoring && handlePinChange(currentPin.slice(0, -1))}
            disabled={isStoring}
          >
          ...
          <Pressable
            style={styles.keypadButton}
            onPress={() => !isStoring && handlePinChange(currentPin + '0')}
            disabled={isStoring}
          >
