...
export function PINVerifyScreen({
  ...
  const handlePinChange = async (value: string) => {
    ...
    if (numericValue.length === 4 && !verifying) {
      setVerifying(true);
      setTimeout(async () => {
        const isValid = await authService.verifyPIN(numericValue);
        setVerifying(false);
  ...
  return (
    ...
    >
      <View style={styles.content}>
        ...
        {biometricEnabled && biometricAvailable && pin.length === 0 && (
        ...
        <View style={styles.keypad}>
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <Pressable
              ...
              onPress={() => !verifying && handlePinChange(pin + num.toString())}
              disabled={verifying}
            >
          ...
          <Pressable
            style={styles.keypadButton}
            onPress={() => !verifying && handlePinChange(pin + '0')}
            disabled={verifying}
          >
